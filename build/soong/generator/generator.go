...
func init() {
	android.RegisterModuleType("kosp_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = kospExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = kospExpandVariables(ctx, dep_file)
	...
	cmd := kospExpandVariables(ctx, String(g.properties.Cmd))
